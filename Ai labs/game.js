...
function create() {
	this.land = this.add.tileSprite(320, 180, 800, 500, 'land'); 
	...
	for (let i = 0; i < maxTrees; i++) {
			...
			do {
					...
					randomY = Phaser.Math.Between(50, 310);
...
function update() {
	...
	if (nearestTree) {
			...
			if (minDistance < cutDistance) {
					...
					this.player.anims.play('chop1', true);

					const cutDistance = 10;
let countdown = 5; // Initialize countdown outside so it's not reset each time
// const treeRespawnDelay = 15000; // 15 seconds delay for tree respawn
